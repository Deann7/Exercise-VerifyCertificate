...
const createFallingStar = (delay) => ({
	...
	animate: {
		...
		x: [0, 80, -80, 0],
		opacity: [0, 1, 1, 0],
		transition: { duration: 4, repeat: Infinity, repeatDelay: 0, ease: "linear", delay: delay },
	},
});
...
const Home = () => {
			...
			if (!querySnapshot.empty) {
				...
				toast.success(
					<>
						...
						<div className="font-semibold">{certificate.name} | "{certificate.title}"</div>
	...
	return (
		<div className="w-full h-auto pb-10 lg:pb-20 overflow-x-clip relative">
				...
				<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 2 }}>
					<form onSubmit={handleSubmit} className="flex flex-col sm:flex-row items-center justify-center gap-4 px-4 mt-6 w-full">
						...
						<button
							type="submit"
							disabled={isLoading}
							className={`w-full sm:w-auto p-3 px-8 rounded-lg shadow-md bg-gradient-to-r from-purple_4 to-blue_3 text-white font-semibold ${isLoading ? "opacity-50 cursor-not-allowed" : "hover:scale-105"}`}
						>
			...
			<Image src="/hexagonLeft.svg" alt="hexagonLeft" className="absolute top-20 left-0 hidden lg:block z-0" width={200} height={200} />
			<motion.img
